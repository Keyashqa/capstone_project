...
def _skill_tool(skill_card: dict) -> str:
    caps = skill_card.get("required_capabilities", [])
    return caps[0].get("tool_name", "") if caps else ""
...
def _deterministic_checks(
    ...
    is_custom: bool = bool(skill_card.get("match_keywords"))
    tool: str = _skill_tool(skill_card)

    issues: list[str] = []

    if is_custom:
        # Phase 3 CUSTOM skill: no closed-platform char limit applies. Check by the
        # skill's own tool — a doc-writer must have saved a doc; any skill must
        # produce non-empty output.
        if not output.strip():
            issues.append("specialist returned no output")
        if tool == "create_doc":
            if not any(t.get("tool") == "create_doc" for t in called_tools):
    ...
    # ── Platform skills (unchanged): enforce the platform's character limit ──────
...
async def verify_work(node_input: dict[str, Any]) -> Any:
    ...
    checks_pass, issues = _deterministic_checks(spec, result, skill_card)
