...
export default function SellSkill({ token, email, onNavigate, onLogout }: Props) {
  ...
  return (
    <div className="app-shell">
      <Sidebar active="sell" email={email} token={token} onNavigate={onNavigate} onLogout={onLogout} />
      ...
      <div className="wallet-main">
        <main className="page-wide">
          ...
          {done ? (
            <div className="glass-panel" style={{ borderColor: 'var(--green)' }}>
              <h3 style={{ marginTop: 0, color: 'var(--green)' }}>✅ Published — “{displayName}” is live</h3>
              <p>
                Skill ID <code>{done.skill_id}</code> · agent <b>@{done.agent_name}</b> · earnings pay
                straight into your <BrandWord text="MPay" /> wallet.
              </p>
              ...
              <div className="actions-row" style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                <button className="agent-hire" onClick={() => onNavigate('marketplace')}>View in Skills Marketplace →</button>
              ...
              <div className="sell-form glass-panel">
                ...
                <div className="sell-field">
                  ...
                  <div className="segmented">
                    <button className={`segmented-btn${tool === 'create_doc' ? ' segmented-btn-active' : ''}`}
                      onClick={() => setTool('create_doc')}>✍️ Writes &amp; saves a Google Doc</button>
                    <button className={`segmented-btn${tool === 'get_doc_content' ? ' segmented-btn-active' : ''}`}
                      onClick={() => setTool('get_doc_content')}>📖 Reads a Google Doc</button>
                  </div>
                </div>
                ...
                {error && <div className="market-note market-note-error" style={{ padding: '.75rem 0' }}>{error}</div>}
              </div>

              <aside className="contribute-summary glass-panel">
                <div>
                  <div className="contribute-summary-eyebrow">Live estimate</div>
                  <div className="owned-name" style={{ marginTop: 4 }}>{displayName.trim() || 'Your skill'}</div>
                </div>

                <div className="agent-pricing">
                  <div className="price-line"><span>Buyer pays per hire</span><span>{fmt$(totalCents)}</span></div>
                  <div className="price-line"><span>Broker commission <em>(10% of completion)</em></span><span>-{fmt$(commissionCents)}</span></div>
                  <div className="price-line price-total"><span>You earn per hire</span><span>{fmt$(youKeepCents)}</span></div>
                </div>

                <button className="agent-hire" disabled={!valid || submitting} onClick={submit} style={{ width: '100%' }}>
                  {submitting ? 'Publishing…' : 'Publish to marketplace'}
                </button>

                <p className="contribute-summary-note">
                  Base fee is non-refundable; the completion fee is refunded to the buyer if your skill's
                  work fails verification. Earnings land directly in your MPay wallet the moment a hire settles.
                </p>
              </aside>
            </div>
          )}
